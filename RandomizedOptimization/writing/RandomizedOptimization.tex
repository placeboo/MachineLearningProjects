...
The N-Queens problem is a discrete optimization task that aims to place $N$ queens on an $N \times N$ chessboard without any queens attacking each other \cite{b1}. The fitness function maximizes the number of non-attacking queen pairs. This problem is interesting because of 
...
I implemented the experiments using the \texttt{mlrose-ky} library, defining a custom fitness function that calculates the non-attacking queen pairs. The experiments were structured as follows:
\begin{enumerate}
...
\rhc\ doesn't struggles with larger problem sizes if allowing more iterations ($\geq 200$) which is against my third hypothesis. It starts with the lowest fitness value and gets stuck at iterations 100-200, indicating it's trapped in local optima before finding better solutions through restarts (Fig. \ref{fig: nqueens-fitness-vs-iteration}). This behavior is expected due to the increasing number of local optima as the board size grows. More iterations with restarts can help \rhc\ explore more space and jump out of the local optimas.
...
\end{figure*}
\ga\ exhibits complex interactions between population size and mutation probability (Fig.\ref{fig: nqueens-performance-metrics-ga}). Larger population sizes do not necessarily guarantee increased fitness, as the impact is also influenced by an optimal mutation probability. However, increasing the population size does lead to longer execution times due to greater diversity in the solution space. This observation supports my hypothesis about \ga's effectiveness in finding quality solutions, while also confirming its higher computational cost. High mutation rates favor exploration but may hurt fitness and convergence for smaller population size (Fig. \ref{fig: nqueens-fitness-curves-ga} where population size = 100). A mutation probability of 0.2 generally yields the best fitness values (Fig. \ref{fig: nqueens-fitness-population-ga} and \ref{fig: nqueens-execution-time-population-ga}), balancing exploration and exploitation.
\begin{figure*}[!t]
\centering
\begin{subfigure}[b]{\subfigwidth}
...
I implemented the experiments using the \texttt{mlrose-ky} library with the built-in FourPeaks fitness function. 
...
\begin{itemize}
    ...
    \begin{itemize}
        \item Fix Parameters: max iteration: 1000; learning rate: 0.06; max attempts: 100; early stopping: True; random seed: 42
        \item \rhc\ Restarts: [5, 10, 20]
...
Analysis of the learning curve, comparing training and validation accuracy across iterations, reveals that \sa\ doesn't benefit from increased sample size. This unexpected underperformance in neural network optimization suggests \sa's limited ability to explore the weight space effectively. The lack of improved performance with larger samples likely stems from constraints in \sa's exploration strategy, possibly due to its cooling schedule or the method of generating neighbor solutions in the high-dimensional space of neural network weights.
